...
public class DirectoryConnector {
	...
	private byte[] sendAndReceiveDatagrams(byte[] requestData) {
		...
		DatagramPacket packetToServer = new DatagramPacket(requestData,requestData.length,this.directoryAddress);	//Creamos el datagrama que vamos a enviar al servidor
		DatagramPacket packetFromServer = new DatagramPacket(responseData,responseData.length);					//Creamos datagrama asociado al buffer de recepción
		
		try {
			this.socket.send(packetToServer);
			this.socket.setSoTimeout(TIMEOUT);
			this.socket.receive(packetFromServer);
			
		}catch(SocketTimeoutException e) {
			boolean received = false;
			
			for(int i = 0; i<MAX_NUMBER_OF_ATTEMPTS && received == false; i++) {
				received = this.resend(packetFromServer);
			}
	...
	boolean resend(DatagramPacket packet) {
		try {
			this.socket.setSoTimeout(TIMEOUT);
	...
	public boolean testSendAndReceive() {
		...
		byte[] loginString = "login".getBytes();
		byte[] response = sendAndReceiveDatagrams(loginString);
		String messageReceived = new String(response,0,response.length);
		if(messageReceived.equals("loginok")){
			success=true;
		}
